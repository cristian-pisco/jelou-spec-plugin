...
describe('jlu-settings', () => {
  ...
  test('empty env value defers to the file and non-boolean strings pass through raw', () => {
    ...
    mkdirSync(join(home, '.jlu'), { recursive: true });
    writeFileSync(userSettingsPath(home), JSON.stringify({ autochain: true }));
    assert.equal(resolveSetting('autochain', { home, environ: { JLU_AUTOCHAIN: '' } }), true);
...
describe('jlu-settings CLI contract', () => {
  test('get autochain prints false with exit 0', () => {
    const home = tempHome();
  ...
  test('get reflects the user file value', () => {
    const home = tempHome();
    mkdirSync(join(home, '.jlu'), { recursive: true });
    writeFileSync(join(home, '.jlu', 'settings.json'), JSON.stringify({ autochain: true }));
    const r = runCli(['get', 'autochain'], home);
    assert.equal(r.status, 0);
