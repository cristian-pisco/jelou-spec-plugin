...
2. **If it does NOT exist**: the task was never synced. Create the macro task
   inline with the session-level ClickUp MCP tools, seeding
   `CLICKUP_TASK.json` from
   `{plugin-root}/jelou/templates/clickup-task.json` per §1 of
   `{plugin-root}/jelou/references/autochain-handoff.md`. Do **not** skip with
   "No ClickUp task associated" — that path silently drops the closure comment
   when the user runs close-task on a never-synced task. After the inline sync
   completes, continue to step 3 below.
...
5. **Compose and post the closure comment** on the macro task. This step
   ...
   - **Do NOT include**: PR URLs (already posted by `/jlu-ship`),
     signature lines, ISO timestamps, test counts, phase
   ...
   is `/jlu-ship`'s responsibility and is already attached as a
   separate comment.
