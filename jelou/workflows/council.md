...
## Step 2 — Resolve workspace and services (report scoping)
...
1. Locate `.spec-workspace.json` from the current directory or up to 5 parent
   ...
   to Step 3 (the report will carry the empty-case-file banner unless the user passed `--context`).
2. Read the service registry at `<SPEC_WS_DIR>/registry/services.yaml`.
3. If the cwd resolves to exactly ONE mapped service, use it directly and record it as `SERVICES_IN_SCOPE`.
...
node <plugin-root>/bin/council.mjs "<idea-or-path>" \
  ...
  [--context <path> ...] [--context <session-dir>/deliberation.md]
```
...
- **Banners (when they apply):**
  ...
  - Inventory shows 0 artifacts included →
    `EMPTY CASE FILE — the judges opined without repo context; pass --context with the relevant SPEC or source files to enrich it`
...
1. Write a **self-sufficient** `<session-dir>/new-task-seed.md` — complete enough that a
   ...
   - the services in scope (`SERVICES_IN_SCOPE` from Step 2);
...
- Script exit ≠ 0 in any round → report nothing for that round; show the error and the
  remediation it suggests (export OPENROUTER_API_KEY, trim --context files, install a
