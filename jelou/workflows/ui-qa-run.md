...
- Optional flags:
  ...
  - `--no-boot` — assume the dev infrastructure is already up and owned by the caller
    (e.g. `/jlu-goal`). Skips the pre-flight resource/port gate, the service
    ...
    Under `--no-boot` the caller (`/jlu-goal`) has already materialized the
...
14b. **Auth gate — probe the session, log in via OTP when invalid.** Runs after boot (the target app must be up) and before Playwright. See `jelou/references/auth-fixtures.md` § "Orchestrated OTP login".
    ...
    **Forbidden under all circumstances:** dispatching the fix-loop for auth failures, editing auth-related env values to force a pass, or inserting/patching session documents in any datastore to mask an auth failure. The **one** sanctioned datastore write is the local session provisioning (step 14c, and step 14b's inline auto-heal for a loopback target), and only under its contract: it runs **after a real login has already succeeded**, derives every field solely by decrypting that real minted/captured cookie (never invented), fails closed (reports, never fabricates), and is never used to mask a *failed login*. The 14b auto-heal does run in response to a re-probe 401 — but only when the login itself succeeded; it bridges a genuinely-valid local session that the native login simply never wrote to `logsM.userSessions`, and the post-provision re-probe is the gateway-truth check that alone gates `healed`. The fix-loop remains barred from all datastore session writes and is never a response to a 401.
...
16. **Parse the JSON reporter output** for failures. Each failure has test title, file path, line, error, attached trace.zip path.
    ...
    **Minimal-input guard (never a thin green, under `/jlu-goal`).** When invoked with `NO_BOOT` set (the goal backstop), also reject a green-but-thin run: if the collected specs never exercise a non-default field type or never populate an auto-generatable reference for any Success Criterion that creates/edits an entity with typed/reference fields (the boolean-column→options-filter shape), do NOT report a clean pass — surface reason `minimal_input_coverage`, name the uncovered field/reference dimensions, and hand them to the orchestrator's Phase 4.5 re-dispatch. A green exit on a one-text-column / zero-filter suite must never be reported as success under goal.
...
| Green but minimal input coverage (under goal) | 1 | `minimal_input_coverage`: suite never exercised a non-default field type / populated reference; dimensions named, routed to Phase 4.5 re-dispatch |
