...
Before releasing a change to any skill, agent, or hook, exercise the working tree in a real session with `/dev-link` (project-scoped skill) or directly: `claude --plugin-dir "$PWD"`. That loads this tree under the `jlu:` namespace, outranking the installed release for that session only. Testing after publishing is testing in production.
