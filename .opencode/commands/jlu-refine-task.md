---
description: Refine approved task spec
agent: build
---
Resolve workflow path in this order:
1. `<HOME>/.config/opencode/jelou/workflows/refine-task.md` (global install preferred; resolve `<HOME>` to an absolute path first)
