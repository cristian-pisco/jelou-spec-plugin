...
- A route segment beyond infrastructure prefixes such as `api` and `v1` that names a resource in a handler, schema, entity, event, or table (e.g., `/workflows`, `/datum`, `/agents`)
...
Apply heuristics in priority order. Stop adding evidence at 5 locations; require at least 3 locations when available, preferring distinct evidence kinds.
