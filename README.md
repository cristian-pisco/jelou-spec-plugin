...
**One spec, three runtimes.** The logic lives once in `jelou/workflows/*.md`; each runtime gets a thin generated shell — Claude Code skills (`skills/`), OpenCode commands (`.opencode/`), and Codex skills + TOML subagents (`.codex/`, generated by `bin/sync-codex.mjs`). The same PreToolUse guards (worker-cap + `.env` hygiene) run on Claude Code (`hooks/hooks.json`), Codex (`.codex/hooks.json`, byte-identical hook contract), and OpenCode (`.opencode/plugins/guard.ts`, reusing the same classifiers).
...
git clone https://github.com/cristian-pisco/jelou-spec-plugin.git
cd jelou-spec-plugin
./setup --host codex          # installs skills (→ ~/.agents/skills), agents/workflows/guards into ~/.codex
```

Then in Codex, invoke the skills by the same hyphen-prefixed names as OpenCode, prefixed with `$`:
...
Use the explicit skill form in Codex (`$jlu-load-context`), or let Codex trigger the skill
implicitly when your request matches its description. If a bare command such as
`jlu-load-context` is entered, the installed `AGENTS.md` fallback treats it as the matching
skill; if Codex says it is not on `PATH`, the Codex install is not loaded. Re-run
`./setup --host codex` or use `bin/install-codex.sh <project>` for a project-local install,
then restart Codex in the target repo.

Codex specifics (see [jelou/references/codex-runtime.md](./jelou/references/codex-runtime.md)):

- **Skills + subagents are generated** from `skills/` and `agents/` by `bin/sync-codex.mjs` (CI enforces sync). Edit the canonical sources, then `npm run sync`.
