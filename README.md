...
- **Autochain (full autonomy from interview to green PRs)**: with the plugin-global `autochain` flag on (`~/.jlu/settings.json`, read and seeded by `bin/jlu-settings.mjs`; default off; `JLU_AUTOCHAIN` env overrides), `/jlu-new-task` and `/jlu-refine-task` create-or-bind the ClickUp task at SPEC approval and hand off inline into `/jlu-execute-task`; after final QA green, execute-task ships inline and dispatches a `jlu-resolve-pr-runner` per open PR (sequentially) to drive review threads, CI, and Sonar to green. Task-green is the AND of every PR verdict, progress persists in the task's `AUTOCHAIN.json` (a dead session re-enters where it stopped), and when green the ClickUp status flips to `PENDING TO PRODUCTION`. Pass a ClickUp URL or id inline on any of the three commands to bind an existing task; `--no-autochain` opts a single invocation out.
...
| `/jlu-new-task [desc] [clickup-url\|id] [--no-autochain]` | Create a new task with spec, worktrees, and affected service detection. With `autochain` on, creates/binds the ClickUp task at SPEC approval and chains inline into `/jlu-execute-task` |
| `/jlu-refine-task [slug] [clickup-url\|id] [--no-autochain]` | Apply a targeted change to an approved spec via structured interview. With `autochain` on, re-syncs ClickUp and re-enters `/jlu-execute-task` when phases changed |
