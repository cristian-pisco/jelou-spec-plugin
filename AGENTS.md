...
- If the user enters a bare `jlu-*` command token (`jlu-load-context`, `jlu-load-context <args>`, "run jlu-load-context", or "use the jlu-load-context skill"), treat it as the matching Codex prompt, not as a shell executable or a `skills/*/SKILL.md` skill.
...
- If no matching prompt exists, stop with: "Codex prompt not installed: `<command>`. Run `./setup --host codex` from jelou-spec-plugin or `bin/install-codex.sh <project>` for a project-local install." Never search `PATH`, package scripts, repo helpers, or `skills/*/SKILL.md` for `jlu-*`.
