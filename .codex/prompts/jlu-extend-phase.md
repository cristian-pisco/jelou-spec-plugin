---
description: "Use when scope changes mid-task — adds new requirements to an in-progress phase via focused interview."
argument-hint: "[task-slug] [phase-number]"
---
Resolve the workflow file in this order, and use the first one that exists:
1. `$CODEX_HOME/jelou/workflows/extend-phase.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/extend-phase/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
