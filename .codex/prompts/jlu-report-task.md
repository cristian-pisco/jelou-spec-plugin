---
description: "Use to check task status — shows progress, blockers, and detects stale worktrees."
argument-hint: "[task-slug]"
---
Resolve the workflow file in this order, and use the first one that exists:
1. `$CODEX_HOME/jelou/workflows/report-task.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/report-task/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
