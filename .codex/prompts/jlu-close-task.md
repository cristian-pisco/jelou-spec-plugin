...
1. `$CODEX_HOME/jelou/workflows/close-task.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/close-task/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
