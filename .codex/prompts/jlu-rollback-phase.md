---
description: "Use when a phase went wrong — resets service worktrees to the last known-good state."
argument-hint: "[task-slug] [phase-number]"
---
Resolve the workflow file in this order, and use the first one that exists:
1. `$CODEX_HOME/jelou/workflows/rollback-phase.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/rollback-phase/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
