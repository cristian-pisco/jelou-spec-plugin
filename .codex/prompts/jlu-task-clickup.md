---
description: "Use to sync task state to ClickUp — creates or updates macro tasks and subtasks from workspace artifacts."
argument-hint: "[task-slug]"
---
Resolve the workflow file in this order, and use the first one that exists:
1. `$CODEX_HOME/jelou/workflows/task-clickup.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/task-clickup/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
