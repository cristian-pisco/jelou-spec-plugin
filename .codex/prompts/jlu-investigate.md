---
description: "Use for research and decision investigation — researching a question, comparing options, or gathering what is known about a topic, persisted as a resumable note."
argument-hint: "<question> [--engine perplexity|fusion]"
---
Resolve the workflow file in this order, and use the first one that exists:
1. `$CODEX_HOME/jelou/workflows/investigate.md` (global install; `$CODEX_HOME` defaults to `~/.codex` — resolve it to an absolute path first).
...
- Do not read or execute `skills/investigate/SKILL.md`; `skills/*/SKILL.md` files are Claude Code entry points, not Codex prompts.
