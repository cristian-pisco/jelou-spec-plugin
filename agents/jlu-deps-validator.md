...
- `<SERVICE_ID>` — the registry service id to validate.
- `<SERVICE_CWD>` — absolute path to that service's checkout.
- `<PLUGIN_ROOT>` — absolute plugin root. Resolve the validator binary from here; you
  cannot derive it yourself, so a missing value is a dispatch bug — report it, never guess.

## How

Run exactly one command and capture its exit code and output:
...
- Command: `node "<PLUGIN_ROOT>/bin/install-dep.mjs" --validate <SERVICE_ID> --cwd <SERVICE_CWD>`
