...
- **Refusing to invent is not refusing to vary input.** Declining to invent routes/testids/endpoints/env-vars does NOT mean sending only the simplest field shape. When a flow creates or edits a typed/reference entity, you MUST exercise a non-default field type and a populated reference per derive-from-spec rule 4b — using only the minimal single-field shape is under-testing, not discipline.
...
- `<EXPECT>` — what a correct test should do when run (default: `red`). One of:
  ...
  - `live` — post-deploy QA: the UI already exists, so a correct test PASSES. Skip step 3h's run entirely — the orchestrator runs the suite itself right after this dispatch; running it twice doubles the wall clock for nothing. Verify compilation (step 3g) and collection (`npx playwright test <file> --list`) only.
...
- `<TASK_DIR>/services/<UI_SERVICE_ID>/e2e/required-env.txt` — one env var name per line, the union of `Env Vars` rows across every flow targeting this UI service, plus `E2E_BASE_URL`. Read by the E2E caller to fail-fast before launching Playwright when a var is unset.
...
1. **`<UI_SERVICE_WORKTREE>/tests/e2e/playwright.config.ts`** — dedicated dir so it does not collide with any existing Vitest suite (e.g. `libs/builder/src/e2e`):
   ...
   The `video` line honors the plugin's E2E video contract: the E2E caller exports
...
4b. **Typed / reference fields → non-default field type + negative coverage (mandatory in BOTH normal and derive-from-spec modes).** When a Success Criterion (or a `user-flow.md` block) creates or edits an entity that has typed or reference fields (a column type, a filter referencing a column, an option set, a typed query parameter), derive at least one step group that exercises a **non-default field type** (e.g. a boolean/options column, not only a free-text column) AND populates an auto-generatable reference (e.g. add a filter that targets that column by id — the boolean-column→options-filter→GUID shape), plus one step group for the **negative / validation-rejection** path (the UI surfaces the error; assert the error state, never skip). Do not collapse every entity to its simplest single-field shape — that is under-testing, not discipline. **In `normal` mode**, if a pre-authored `user-flow.md` block edits such an entity but describes only a single default-field happy path, you MUST still emit this coverage by varying input within the declared surface (per the "Refusing to invent is not refusing to vary input" rule above); if the block lacks the data to do so faithfully (e.g. no declared selector for the non-default field), escalate `STATUS: NEEDS_CONTEXT, missing: non-default-field / negative coverage for <entity> in user-flow.md` rather than generating a thin happy-path-only spec.
