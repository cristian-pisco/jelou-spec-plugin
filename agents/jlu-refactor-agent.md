...
2. **The files in the `Files Modified` union** — refactors must match the patterns already visible there, and those files also tell you where complexity belongs.
3. **Test files** provided in your prompt — read enough of them to understand what the public contract is. You preserve this contract.
...
4. Names that break from the naming already used in the same file or module.
...
- Match the style of the surrounding code on every changed line.
